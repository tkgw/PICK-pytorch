...
class GraphLearningLayer(nn.Module):
    ...
    def forward(self, x: Tensor, adj: Tensor, box_num: Optional[Tensor] = None) -> Tuple[Tensor, Optional[Tensor]]:
        ...
        # (B, N, learning_dim) <- (B, N, in_dim)
        x_hat: Tensor = self.projection(x)
        B, N, learning_dim = x_hat.size()

        # (B, N, N, learning_dim)
        x_i: Tensor = x_hat.unsqueeze(2).expand(B, N, N, learning_dim)
        x_j: Tensor = x_hat.unsqueeze(1).expand(B, N, N, learning_dim)
        # (B, N, N, learning_dim)
        distance: Tensor = torch.abs(x_i - x_j)
        ...
        # (B, N, N)
        out: Tensor = F.leaky_relu(torch.einsum('bijd, d->bij', distance, self.learn_w))

        # for numerical stability, due to softmax operation mable produce large value
        max_out_v, _ = out.max(dim=-1, keepdim=True)
        out = out - max_out_v

        soft_adj: Tensor = torch.exp(out).mul(adj)

        soft_adj = soft_adj / (soft_adj.sum(dim=-1, keepdim=True) + 1e-10)
...
class GCNLayer(nn.Module):
    ...
    def forward(self, x: Tensor, alpha: Tensor, adj: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        # (B, N, N, in_dim)
        x_i: Tensor = x.unsqueeze(2).expand(B, N, N, in_dim)
        x_j: Tensor = x.unsqueeze(1).expand(B, N, N, in_dim)
        ...
        H: Tensor = F.relu(x_i + x_j + alpha + self.bias_h)

        # update node embedding x, （B, N, out_dim）
        AH: Tensor = torch.einsum('bij, bijd-> bid', adj, H)
...
class GLCN(nn.Module):
    ...
    def __init__(self,
        ...
        modules: List[GCNLayer] = []
        in_dim_cur = in_dim
        for _ in range(num_layers):
            modules.append(GCNLayer(in_dim_cur, out_dim))
            in_dim_cur = out_dim
        self.gcn = nn.ModuleList(modules)

        self.alpha_transform = nn.Linear(6, in_dim, bias=False)

    def forward(self, x: Tensor, rel_features: Tensor, adj: Tensor, box_num: Optional[Tensor]) -> Tuple[Tensor, Tensor, Optional[Tensor]]:
        """

        :param x: nodes embedding, (B, N, in_dim)
